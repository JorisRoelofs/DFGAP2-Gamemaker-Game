...
for(i = array_length_1d(statName) - 1; i >= 0; i-=1)
{
    draw_set_color(c_black);
    //draw_text(x, y+drawHeight[1], statName[i]);
    draw_rectangle(x-drawWidth, y+drawHeight[2], x+drawWidth, y+drawHeight[3],false);
    if(statValue[i] < 0.2 || statValue[i] > 0.8) draw_set_color(c_red);
    else draw_set_color(c_green);
    draw_rectangle(x-drawWidth, y+drawHeight[2], x-drawWidth+(2*drawWidth*statValue[i]), y+drawHeight[3],false);
